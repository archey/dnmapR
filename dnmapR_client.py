    ...
    def dataReceived(self, line):
        ...
        # dataReceived does not wait for end of lines, CR, or LF
            ...
            if check_clean(line):
                ...
                if nameReturnCode >= 0:
                    # nmap ended ok
                    if os.path.isfile(nmapOutputFile+".xml") and os.path.isfile(nmapOutputFile+".gnmap") and os.path.isfile(nmapOutputFile+".nmap"):
                        with open(nmapOutputFile+".xml","r") as f:
                            XMLData=f.read()
                        with open(nmapOutputFile+".gnmap","r") as f:
                            GNmapData=f.read()
                        with open(nmapOutputFile+".nmap","r") as f:
                            NmapData=f.read()
                        ...
                        line = rawNmapOutput+xml_linesep+XMLData+gnmap_linesep+GNmapData
                        self.sendLine(line)
